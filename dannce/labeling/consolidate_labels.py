...
folders = [f for f in os.listdir(annotdir) if os.path.isdir(os.path.join(annotdir,f))]
...
sortedlist = sorted(list(manifest.keys()),key=lambda x: int(x.lstrip('sample').split('_')[0]))

# Get labels
nummarkers = len(markernames)
numcams = len(camnames)
marker_i = np.zeros((len(sortedlist)//numcams,nummarkers,numcams))
marker_coords = np.zeros((len(sortedlist)//numcams,nummarkers,numcams,2))*np.nan
    ...
    for j in range(numcams):
        ...
        if os.path.exists(fpath):
            ...
            with open(os.path.join(fpath,jj[0])) as f:
                data = json.load(f)
            data = data['answers'][0]['answerContent']['annotatedResult']['keypoints']
            if len(data) > 0:
                #Save the name of this image file so that it can be removed from the original manifest
                good.append(t)
...
for j in range(numcams):
    thiscam = camnames[j]
    sio.savemat(os.path.join(RESULTSDIR,'{}_manlabels.mat'.format(thiscam)),
