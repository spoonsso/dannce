...
def sample_grid(im: np.ndarray, projPts: np.ndarray, method: Text = "linear"):
    ...
    # dimension, matrix-wise (i.e. from the top of the image)
        ...
        np.clip(projPts[:, 0], 0, im.shape[0] - 1, out=projPts[:, 0])
        np.clip(projPts[:, 1], 0, im.shape[1] - 1, out=projPts[:, 1])
...
def sample_grid_torch_nearest(
    ...
    feats = torch.from_numpy(im.copy()).to(device=device)
    grid = projPts
    ...
    grid = grid.round().type(torch.long)
    im_xr = torch.clamp(grid[:, 0], 0, fw - 1)
    im_yr = torch.clamp(grid[:, 1], 0, fh - 1)
