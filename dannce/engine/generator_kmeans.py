...
class DataGenerator(keras.utils.Sequence):
	...
	def __init__(
		...
		dim_in=(32, 32, 32), n_channels_in=1, dim_out=(32, 32, 32),
		n_channels_out=1, out_scale=5, shuffle=True, camnames=[],
		crop_width=(0, 1024), crop_height=(20, 1300), tilefac=1,
		bbox_dim=(32, 32, 32), samples_per_cluster=0, training=True,
		vidreaders=None):
		...
		# This is only used when serving/targeting 2D, and even so is deprecated
...
class DataGenerator_3Dconv_kmeans(DataGenerator):
	...
	def __init__(
		...
		dim_out=(32, 32, 32), n_channels_out=1, out_scale=5, shuffle=True,
		camnames=[], crop_width=(0, 1024), crop_height=(20, 1300),
		tilefac=1, bbox_dim=(32, 32, 32), vmin=-100, vmax=100, nvox=32,
		...
		DataGenerator.__init__(
			...
			n_channels_in, dim_out, n_channels_out, out_scale, shuffle,
			camnames, crop_width, crop_height, tilefac, bbox_dim,
...
class DataGenerator_3Dconv_frommem(keras.utils.Sequence):
	...
	def __init__(
		...
		chan_num=3, shuffle=True, expval=False, xgrid=None, var_reg=False):
		"""Initialize data generator."""
		self.list_IDs = list_IDs
		self.data = data
		self.labels = labels
