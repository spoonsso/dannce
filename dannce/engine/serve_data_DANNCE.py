# Boilerplate for routines for reading/structuring input data
import numpy as np
...
def prepare_data(CONFIG_PARAMS, com_flag = True, nanflag = True):
	"""
	Given a set of config params, assemble necessary data structures and return them -- tailored to center of mass finding
		That is, we are refactoring to get rid of unneeded data structures (i.e. data 3d)
	"""
	...
	# Collect data labels and matched frames info. We will keep the 2d labels here just because we could in theory use this for training later.
	...
	if 'calib_file' in list(CONFIG_PARAMS.keys()):
		...
		for i in range(len(CONFIG_PARAMS['CAMNAMES'])):
			test = sio.loadmat(os.path.join(CONFIG_PARAMS['CALIB_DIR'],CONFIG_PARAMS['calib_file'][i]))
			cameras[CONFIG_PARAMS['CAMNAMES'][i]] = {'K':test['K'], 'R':test['r'],'t':test['t']}
...
def do_retriangulate(this_com,j,k,uCamnames, camera_mats):
	"""
	If cameras parameters have been updated since finding COM, COM should be re-trianngulated
	"""
...
def prepare_COM(comfile,datadict,comthresh=0.01, weighted=True, retriangulate=False,
	camera_mats = None, conf_rescale = None, method='mean'):
	"""
	Loads COM file, replaces 2D coordinates in datadict with the preprocessed COM coordinates, returns dict of 3d COM coordinates

	Thresholds COM predictions at comthresh w.r.t. saved pred_max values. Averages only the 3d coords for camera pairs that both meet thresh.
		Returns nan for 2d COM if camera does not reach thresh. This should be detected by the generator to return nans such that bad camera
	...
	_, idx = np.unique(camnames, return_index=True) # Because I repeat cameras to fill up 6 camera quota, I need grab only the unique names
		...
		if key in datadict.keys():
			...
			for j in range(len(uCamnames)):
				for k in range(j+1, len(uCamnames)):
					...
					if this_com[uCamnames[j]]['pred_max'] > comthresh and this_com[uCamnames[k]]['pred_max'] > comthresh:
						com3d[:,cnt] = this_com['triangulation']['{}_{}'.format(uCamnames[j]
																				,uCamnames[k])]
...
def prepare_com3ddict(datadict_3d):
	"""
	Call this when using ground truth 3d anchor points that do not need to be loaded in via a special com file -- just need to take the mean
...
def addCOM(d3d_dict, c3d_dict):
	"""
	For JDM37 data and its ilk, the data are loaded in centered to the animal center of mass (Because they were predictions from the network)

	We need to add the COM back in, because durign training everything gets centered to the true COM again
	"""
...
def remove_samples(s, d3d, mode='clean',auxmode=None):
	"""
	Filter data structures for samples that mean inclusion criteria (i.e. mode)

	mode == 'clean' means only use samples in which all ground truth markers are recorded
...
def add_experiment(experiment, samples_out, datadict_out, datadict_3d_out, com3d_dict_out, samples_in, datadict_in, datadict_3d_in, com3d_dict_in):
	"""
	append *_in variables to out variables, after appending the experiment number to the front of keys
	"""
