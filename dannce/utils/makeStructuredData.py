...
for i in range(len(cnames)):
    ...
    _, inds, _ = np.intersect1d(mframes['data_sampleID'], pred['sampleID'], return_indices=True)
    ...
    if len(sys.argv) > 2 and sys.argv[2] == 'True':
        ...
        for m in range(len(markernames)):
            if smax < 0:
                ...
                mocap[markernames[m]] = np.concatenate((mocap[markernames[m]],np.zeros((np.abs(smax),3))*np.nan),axis=0)
            elif smax > 0:
                # To shift positively, we need to add NaN
                mocap[markernames[m]] = np.concatenate((np.zeros((smax, 3))*np.nan, mocap[markernames[m]]), axis=0)

# save everything as matlab struct
pfile = os.path.join(RESULTSDIR, 'predictions.mat')
pdict = {'fullpath': fullpath,
         'name': name,
         'session': date,
         'netname': netname,
