# Randomly samples videos to generate images for labeling
#
# Usage: python generate_labels.py path_to_config_file num_images_per_camera path_to_s3_folder
#
import numpy as np
import imageio
...
partition = {}
partition['train'] = np.random.choice(samples_,size=(int(sys.argv[2]),),replace=False)
...
with open(os.path.join(CONFIG_PARAMS['RESULTSDIR'],'dataset.manifest'),'w') as f:
	for it in allc:
		fname = it.strip()
		fname = fname.split('/')[-1]
