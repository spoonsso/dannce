...
samples_, datadict_, datadict_3d_, data_3d_, cameras_ = serve_data.prepare_data(CONFIG_PARAMS['experiment'])
datadict_, com3d_dict_ = serve_data.prepare_COM(CONFIG_PARAMS['experiment']['COMfilename'],datadict_,
											   comthresh=CONFIG_PARAMS['comthresh'],weighted=CONFIG_PARAMS['weighted'],
											   retriangulate=False,camera_mats=cameras_,method=CONFIG_PARAMS['com_method'])

# Need to cap this at the number of samples included in our COM finding estimates
...
samples_, data_3d_ = serve_data.remove_samples_com(samples_,data_3d_, com3d_dict_,rmc=True)

print("Detected {} bad COMs and removed the associated frames from the dataset".format(pre-len(samples_)))
...
if len(ke) == 1:
	...
	framecnt = int(ke[1].split('/')[-1].split('.')[0]) - int(ke[0].split('/')[-1].split('.')[0])
	print("Videos contain {} frames".format(framecnt))

# Parameters
valid_params = {'dim_in': (CONFIG_PARAMS['INPUT_HEIGHT'],CONFIG_PARAMS['INPUT_WIDTH']),
...
# Generators
valid_generator = DataGenerator_3Dconv_kmeans(partition['valid_sampleIDs'], datadict, datadict_3d, cameras,
...
model = CONFIG_PARAMS['net'](CONFIG_PARAMS['loss'],CONFIG_PARAMS['lr'],
							 ...
							 CONFIG_PARAMS['N_CHANNELS_OUT'], len(camnames[0]),
...
def evaluate_ondemand(start_ind,end_ind,valid_gen):
		...
		for i in range(start_ind,end_ind):
			...
			if framecnt is not None:    
				...
				vids, lastvid_, currvid_ = processing.close_open_vids(lastvid,
															   lastvid_,
															   currvid,
															   currvid_,
															   framecnt,
...
if CONFIG_PARAMS['EXPVAL']:
	...
	p_n = savedata_expval(RESULTSDIR + 'save_data_AVG.mat',
						write=True,
						data=save_data,
						tcoord=False,
						pmax=True)
	...
	p_n = savedata_tomat(RESULTSDIR + 'save_data_MAX.mat',
						CONFIG_PARAMS['VMIN'],
						CONFIG_PARAMS['VMAX'],
						CONFIG_PARAMS['NVOX'],
						write=True,
