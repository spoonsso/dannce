...
def batch_rgb2gray(imstack):
	"""
	batch dimension is first. Convert to gray image-wise
	"""
...
def return_tile(imstack,fac=2):
	...
	out = np.zeros((imstack.shape[0]*fac*fac,height,width,imstack.shape[3]),'float32')
	cnt = 0;
...
def tile2im(imstack,fac=2):
	...
	out = np.zeros((imstack.shape[0]//(fac*fac),height*fac,width*fac,imstack.shape[3]),'float32')
	cnt = 0
...
def downsample_batch(imstack,fac=2, method='PIL'):
	...
	"""
	out = np.zeros((imstack.shape[0],imstack.shape[1]//fac,imstack.shape[2]//fac,imstack.shape[3]),'float32')
	if method == 'PIL':
		if out.shape[-1] == 3:
			# this is just an RGB image, so no need to loop over channels with PIL
			for i in range(imstack.shape[0]):
				out[i] = np.array(PIL.Image.fromarray(imstack[i].astype('uint8')).resize((out.shape[2],out.shape[1]),resample=PIL.Image.LANCZOS))
		else:
			for i in range(imstack.shape[0]):
				for j in range(imstack.shape[3]):
					out[i,:,:,j] = np.array(PIL.Image.fromarray(imstack[i,:,:,j]).resize((out.shape[2],out.shape[1]),resample=PIL.Image.LANCZOS))
	...
	# THis actually appeared slower than my loops, oddly...
	# return dsm(imstack,(1,fac,fac,1))
...
def batch_maximum(imstack):
	...
	for i in range(imstack.shape[0]):
		if np.isnan(imstack[i,0,0]):
			...
			ind = np.unravel_index(np.argmax(np.squeeze(imstack[i]), axis=None), np.squeeze(imstack[i]).shape)
			maxpos[i,0] = ind[1]
			maxpos[i,1] = ind[0]

	return maxpos

def generate_readers(viddir,camname, minopt=0, maxopt = 300000,pathonly=False,extension='.mp4'):
	"""
	...
	for i in range(len(mp4files)):
		if pathonly:
			...
			out[mp4files_scrub[i]] = imageio.get_reader(os.path.join(viddir,mp4files[i]))

	return out

def generate_readers_wrap(CONFIG_PARAMS,vid_dir_flag=False):
	"""
	This is a new version of generate_readers that calls the old version. This higher level wrapper accounts for the use case
		wherein the numbered subdirectories have not been removed from the video folders
	"""
	vids = {}
	for i in range(len(CONFIG_PARAMS['CAMNAMES'])):
		...
		vids[CONFIG_PARAMS['CAMNAMES'][i]] = generate_readers(CONFIG_PARAMS['viddir'],
...
def cropcom(im,com,size=512):
	...
	if minlim_r < 0:
		#print(out.shape)
		out = np.concatenate((np.zeros((abs(minlim_r),out.shape[1],dim)),out),axis=0)
	if maxlim_r > im.shape[0]:
		#print(out.shape)
		out = np.concatenate((out,np.zeros((maxlim_r-im.shape[0],out.shape[1],dim))),axis=0)
	if minlim_c < 0:
		#print(out.shape)
		out = np.concatenate((np.zeros((out.shape[0],abs(minlim_c),dim)),out),axis=1)
	if maxlim_c > im.shape[1]:
		#print(out.shape)
		out = np.concatenate((out,np.zeros((out.shape[0],maxlim_c-im.shape[1],dim))),axis=1)

	return out

def write_config(resultsdir,configdict,message,filename='modelconfig.cfg'):
	"""
	A much more customizable configuration writer. Accepts a dictionary of key-value pairs and just writes them all to file,
...
def collect_predictions_2D(model, valid_generator):
	"""
	Takes a Keras model with pre-loaded trained weights, and a data generator to generate images and ground truth labels

	Returns the locations of the peaks for each keypoint, as predicted by the model for a set of input images,
		Peak positions for the ground truth are also returned  
	"""
	num_ims = valid_generator.__len__()

	firstim = valid_generator.__getitem__(0) # Get first object, to be used to initialize other variables with the correct size
	...
	for i in range(num_ims):
		...
		for j in range(batch_size):
			...
			for k in range(pred_single.shape[-1]):
				...
				valid_peak[cnt,:,k] = np.max(np.max(pred_single[:,:,:,k],axis=1),axis=1)
			cnt = cnt + 1
...
def err_vs_kept(err,pmax,bins=100):
	"""
	Plot mean distance error as a function of the proportion of frames kept, as the threshold varies
	"""
...
def err_vs_kept_percentile(err,pmax,bins=100,pct=50):
	"""
	Calculate distance error at a set percentile, as a function of the proportion of frames kept, as the peak threshold varies
	"""
	thresh_range = np.linspace(0,1,bins)
	kept = np.zeros((bins,))
	error = np.zeros((bins,))
...
def plot_markers_2d(im,markers,newfig=True):
	...
	for mark in range(markers.shape[-1]):
		ind = np.unravel_index(np.argmax(markers[:,:,mark], axis=None), markers[:,:,mark].shape)
...
def plot_markers_3d(stack,nonan=True):
	"""
	takes a stack of 3d probability maps, and returns the 3d coordinates for each of the peaks
	"""
	x = []
	y = []
	z = []
	for mark in range(stack.shape[-1]):
		ind = np.unravel_index(np.argmax(stack[:,:,:,mark], axis=None), stack[:,:,:,mark].shape)
...
def write_unprojected_grids(imstack,fn):
	"""
	From the 3D unprojected grid input to a network, write the grids to tif files for visualization.

	input--
		imstack: shape (nvox, nvox, nvox, 9). Last index corresponds to 3 cameras w/ 3 channels (i.e. RGB)
...
def moment_3d(im,mesh,thresh=0):
	...
	inputs--
		im: 3d volume confidence map, one for each channel (marker), i.e. shape (nvox,nvox,nvox,nchannels)
	...
	for mark in range(im.shape[3]):
		im_norm = (im[:,:,:,mark]*(im[:,:,:,mark]>=thresh))/np.sum(im[:,:,:,mark]*(im[:,:,:,mark]>=thresh)) #get normalized probabilities
...
def animate_predictions(preds):
	...
	fig, ((ax1,ax2,ax3),(ax4,ax5,ax6)) = plt.subplots(2,3)#,dpi=300,figsize=(2,2))
	aa = ax1.imshow(np.max(np.mean(preds[0,:,:,:,:],axis=2),axis=2))
	ax1.axis('off')
	ax1.set_title('XY')
	bb = ax2.imshow(np.max(np.mean(preds[0,:,:,:,:],axis=0),axis=2)[:,::-1].T)
	ax2.axis('off')
	ax2.set_title('XZ')
	cc = ax3.imshow(np.max(np.mean(preds[0,:,:,:,:],axis=1),axis=2)[:,::-1].T)
	ax3.axis('off')
	ax3.set_title('YZ')

	# plot just the peak values now
	#dd = ax4.imshow(np.nanmax(np.nanmean(ims[1][0,:,:,:,:],axis=2),axis=2))
	dd_1, = ax4.plot([0],[0],'or',mfc='none')#,markersize=3,markeredgewidth=0.5)
	dd_2, = ax4.plot([0],[0],'ob',mfc='none')#,markersize=3,markeredgewidth=0.5)
	ax4.set_facecolor('k')
	ax4.set_xlim([0, 96])
	ax4.set_ylim([0, 96])
	ax4.axis('off')
	ax4.set_title('XY')
	#ee = ax5.imshow(np.nanmax(np.nanmean(ims[1][0,:,:,:,:],axis=0),axis=2)[:,::-1].T)
	ee_1, = ax5.plot([0],[0],'or',mfc='none')#,markersize=3,markeredgewidth=0.5)
	ee_2, = ax5.plot([0],[0],'ob',mfc='none')#,markersize=3,markeredgewidth=0.5)
	ax5.set_facecolor('k')
	ax5.set_xlim([0, 96])
	ax5.set_ylim([0, 96])
	ax5.axis('off')
	ax5.set_title('XZ')
	#ff = ax6.imshow(np.nanmax(np.nanmean(ims[1][0,:,:,:,:],axis=1),axis=2)[:,::-1].T)
	ff_1, = ax6.plot([0],[0],'or',mfc='none')#,markersize=3,markeredgewidth=0.5)
	ff_2, = ax6.plot([0],[0],'ob',mfc='none')#,markersize=3,markeredgewidth=0.5)
	ax6.set_xlim([0, 96])
	ax6.set_ylim([0, 96])
	...
	for i in range(preds.shape[0]):
		...
		#ax4.cla()
		#dd.set_data(np.nanmax(np.nanmean(ims[1][i,:,:,:,:],axis=2),axis=2))
		#ee.set_data(np.nanmax(np.nanmean(ims[1][i,:,:,:,:],axis=0),axis=2)[:,::-1].T)
		#ff.set_data(np.nanmax(np.nanmean(ims[1][i,:,:,:,:],axis=1),axis=2)[:,::-1].T)
		
		x, y, z = plot_markers_3d(ims[1][i,:,:,:,:],nonan=False)
		#x, y, z = moment_3d(ims[1][i,:,:,:,:],(x_coord_3d, y_coord_3d, z_coord_3d))
		dd_1.set_xdata(x)
		dd_1.set_ydata(y)
		ee_1.set_xdata(x)
		ee_1.set_ydata(z)
		...
		#x, y, z = moment_3d(pred_single[i,:,:,:,:],(x_coord_3d, y_coord_3d, z_coord_3d),thresh=0.25)
		dd_2.set_xdata(x)
		dd_2.set_ydata(y)
		ee_2.set_xdata(x)
		ee_2.set_ydata(z)
...
def get_peak_inds(map_):
	"""
	Given an n-d map -- typically of some confidence distribution for the position of a feature -- returns the
		indices of the peak value
	"""
...
def get_marker_peaks_2d(stack):
	"""
	Given a stack of 2d confidence maps, return the concatenated coordinates of all peaks for each map/marker
	"""
...
def savedata_expval(fname,write=True, data=None, num_markers=20, tcoord=True, pmax=False):
	if data is None:
		f = open(fname,'rb')
		data = cPickle.load(f)
		f.close()
	...
	# p_max = np.zeros((list(data.keys())[-1]+1,*data[0]['pred_max'].shape))
	...
	if write and data is None:
		sio.savemat(fname.split('.pickle')[0] + '.mat',
...
def savedata_tomat(fname, vmin, vmax, nvox, write=True, data=None, 
	num_markers=20, tcoord = True,tcoord_scale=True, addCOM = None):
	"""
	...
	if addCOM is not None:
		#we use the passed comdict to add back in the com, this is useful if one wnats to bootstrap on these values
		#for COMnet or otherwise
		for i in range(len(sID)):
			pred_out_world[i] = pred_out_world[i] + addCOM[int(sID)][:,np.newaxis]

	if write and data is None:
		sio.savemat(fname.split('.pickle')[0] + '.mat',
			   {'pred': pred_out_world,'data':t_coords,'p_max':p_max, 'sampleID': sID, 'log_pmax':log_p_max})
	elif write and data is not None:
		sio.savemat(fname,
			   {'pred': pred_out_world,'data':t_coords,'p_max':p_max, 'sampleID': sID, 'log_pmax':log_p_max})		
...
def spatial_expval(map_):
	"""
	Calculates the spatial expected value of the input
	Note there is probably underflow here that I am ignoring, because this doesn't need to be *that* accurate
	"""
...
def spatial_var(map_):
	"""
	Calculates the spatial variance of the input
	"""
