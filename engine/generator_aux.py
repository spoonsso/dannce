...
class DataGenerator_downsample(keras.utils.Sequence):
	...
	def __init__(self, list_IDs, labels, vidreaders, batch_size=32, dim_in=(32,32,32), n_channels_in=1, 
		...
		if immode == 'video':
			self.extension = '.' + list(vidreaders[camnames[0]].keys())[0].rsplit('.')[-1]
	...
	def load_tif_frame(self,ind,camname):
		...
		return imageio.imread(os.path.join(self.vidreaders[camname],'{}.tif'.format(ind)))


	def __data_generation(self, list_IDs_temp):
		'Generates data containing batch_size samples' # X : (n_samples, *dim, n_channels)
		# Initialization
		X = np.empty((self.batch_size*len(self.camnames), *self.dim_in, self.n_channels_in),dtype='float32')

		# We'll need to transpose this later such that channels are last, but initializaing the array this ways gives us
		# more flexibility in terms of user-defined array sizes
		y = np.empty((self.batch_size*len(self.camnames), self.n_channels_out, *self.dim_out), dtype='float32')
		...
		for i, ID in enumerate(list_IDs_temp):
			...
			# Store sample
				if self.immode == 'video':
					X[cnt] = self.load_vid_frame(self.labels[ID]['frames'][camname], camname, self.preload, self.extension)[self.crop_height[0]:self.crop_height[1],self.crop_width[0]:self.crop_width[1]]
				elif self.immode == 'tif':

					X[cnt] = self.load_tif_frame(self.labels[ID]['frames'][camname], camname)[self.crop_height[0]:self.crop_height[1],self.crop_width[0]:self.crop_width[1]]

				else:
					raise Exception('Not a valid image reading mode')


				# Labels will now be the pixel positions of each joint. Here, we convert them to
				...
				if self.immode == 'video':
					...
					# DIRTY: HERE WE ASSUME TIF MODE IS ONLY FOR PRE-DOWNSAMPLED SAVED TIFS
				...
				if self.labelmode == 'prob':
					# Only do this if we actually need the labels -- this is too slow otherwise
					for j in range(self.n_channels_out):
						# It's faster just to use the Gaussian pdf eq.
						#y[i,j] = norm.pdf(x_coord,loc=this_y[0,j],scale=self.out_scale)*norm.pdf(y_coord,loc=this_y[1,j],scale=self.out_scale)
		...
		# After we downsample to probabiltiy distributions, we should rescale the maximum back to 1
		y = np.transpose(y,[0,2,3,1])

		if self.downsample > 1:
			X = processing.downsample_batch(X,fac=self.downsample,method=self.dsmode)
			if self.labelmode == 'prob':
				y = processing.downsample_batch(y,fac=self.downsample,method=self.dsmode)
				y /= np.max(np.max(y,axis=1),axis=1)[:,np.newaxis,np.newaxis,:]
		if self.downsample == 1 and self.labelmode == 'prob' and self.immode == 'tif': #Again, we sloppily assume that tifs are already downsampled

			y = processing.downsample_batch(y,fac=self.downsample*2,method=self.dsmode)
	...
	def save_for_dlc(self,imfolder, ext = '.png',full_data=True):
		# Generate data
		# The full_data flag is used so that one can write only the coordinates and not the images, if desired
			cnt = 0
			list_IDs_temp = self.list_IDs
			dsize = self.labels[list_IDs_temp[0]]['data'][self.camnames[0]].shape
			allcoords = np.zeros((len(list_IDs_temp)*len(self.camnames),dsize[1],3),dtype='int')
			fnames = []
			...
			for i, ID in enumerate(list_IDs_temp):
				for camname in self.camnames:
					if full_data:
						X = self.load_vid_frame(self.labels[ID]['frames'][camname], camname, self.preload, self.extension)[self.crop_height[0]:self.crop_height[1],self.crop_width[0]:self.crop_width[1]]
					...
					if self.downsample > 1:
						X = processing.downsample_batch(X[np.newaxis,:,:,:],fac=self.downsample,method='dsm')
						this_y = np.round(this_y/2).astype('int')
						if full_data:
							imageio.imwrite(imfolder + 'sample{}_'.format(ID) + camname + ext,X[0].astype('uint8'))
					else:
						if full_data:
							imageio.imwrite(imfolder + 'sample{}_'.format(ID) + camname + ext,X.astype('uint8'))

					allcoords[cnt,:,0] = np.arange(dsize[1])
					allcoords[cnt,:,1:] = this_y.T

					relpath = imfolder.split('/')[-2]
					relpath = '../' + relpath + '/sample{}_'.format(ID) + camname + ext
					fnames.append(relpath)

					cnt = cnt + 1

			sio.savemat(imfolder + 'allcoords.mat',{'allcoords': allcoords,
